...
扫雷是本合集中的第三个游戏。玩家需要翻开所有安全格，并利用数字推断地雷位置。当前实现使用内置约束求解器验证棋盘，保证每局都能在不猜测的情况下完成，同时提供逻辑提示、三种经典难度、首次点击安全、双语界面、多套主题和各难度最佳时间记录。
...
| 初级游戏与求解器提示 | 游戏设置 |
| --- | --- |
| ![扫雷初级游戏与求解器提示](../../docs/images/minesweeper/gameplay.png) | ![扫雷游戏设置](../../docs/images/minesweeper/settings.png) |
...
当一个已翻开数字周围的旗帜数量与数字相等时，再次左键点击该数字会翻开其余相邻格。如果旗帜位置错误，快捷展开可能直接触发未标记的地雷。问号只用于记录不确定位置，不计入周围旗帜数；问号格可以直接翻开，也会被空白区域连锁展开。
